...
function ContactSidebar({ profile, currentUserId, agentId }: {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        {contactSent ? (
          ...
          <div className="text-center py-6">
            ...
            <Link href={`/login?redirect=/agents/${toAgentSlug(profile?.full_name, agentId)}`}
      ...
      <Link href="/agents" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors justify-center py-2">
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (idFromOldSlug) {
        ...
        if (!slugLookup) { router.replace("/agents"); return; }
            ...
            {isAgency ? (
              <div className="bg-card border border-border rounded-2xl overflow-hidden mb-8">
                ...
                <div className="relative h-48 sm:h-64 overflow-hidden">
                  {profile.cover_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={profile.cover_url} alt={`${profile.full_name} cover photo`} className="w-full h-full object-cover" />
                  ) : (
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                    <div className="relative flex-shrink-0">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-primary/15 overflow-hidden flex items-center justify-center shadow-lg shadow-primary/20">
                        {profile.avatar_url
                          ? // eslint-disable-next-line @next/next/no-img-element
                            <img src={profile.avatar_url} alt={profile.full_name ?? "Agent avatar"} className="w-full h-full object-cover" />
                          : <span className="text-primary font-extrabold text-3xl">{initials}</span>}
                      </div>
                    </div>
