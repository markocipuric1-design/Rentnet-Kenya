  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-10">
        ...
        {/* Onboarding banner — shown to new users who haven't completed their profile */}
        {!profile.avatar_url && !profile.phone && listings.length === 0 && (
          <div className="bg-primary/5 border border-primary/20 rounded-2xl px-5 py-4 mb-6">
            <p className="text-sm font-bold text-primary mb-2 flex items-center gap-2">
              <Zap className="h-4 w-4" /> Welcome to Rentnet — let's get you set up
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {[
                { done: !!profile.avatar_url, label: "Add a profile photo", href: "#", hint: "Click your avatar above" },
                { done: !!profile.phone, label: "Add your phone number", href: "#", hint: "Edit profile to add contact" },
                { done: listings.length > 0, label: "Post your first listing", href: "/post-listing", hint: "Start attracting buyers & renters" },
              ].map((step) => (
                <div key={step.label} className={`flex items-start gap-2.5 rounded-xl px-3 py-2.5 ${step.done ? "opacity-50" : "bg-card border border-border"}`}>
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${step.done ? "bg-emerald-500/20 text-emerald-600" : "bg-primary/10 text-primary"}`}>
                    {step.done
                      ? <CheckCircle className="h-3 w-3" />
                      : <ArrowRight className="h-3 w-3" />}
                  </div>
                  <div>
                    <p className={`text-xs font-semibold ${step.done ? "line-through text-muted-foreground" : "text-foreground"}`}>{step.label}</p>
